function varargout = stage2_uav_planner(varargin)
...
BatteryLife_temp = ones(nSalesmen,30)*temp_batteryLife; % limitation for one worker is 20 battery life
...
%%        
        for s = 1:nSalesmen
            ...
            %% add start time, after asv moved to the first charging spot
            asv1_travel_time = userConfig.asv_travel(1);
            asv2_travel_time = userConfig.asv_travel(2);
            asv1_first_charger_spot = asv1_travel_time.data(1,2);   % wait_asv_move_time 
            asv2_first_charger_spot = asv2_travel_time.data(1,2);
            
            if userConfig.asv1_is_closer_to_center_than_asv2 == 1   % uav1 carry two uav
                if s == 1
                    wait_asv_move_time = asv1_first_charger_spot;
                elseif s == 2
                    wait_asv_move_time = asv1_first_charger_spot;
                elseif s == 3 
                    wait_asv_move_time = asv2_first_charger_spot;
                end              
            else                                                    % uav2 carry two uav
                if s == 1
                    wait_asv_move_time = asv1_first_charger_spot;
                elseif s == 2
                    wait_asv_move_time = asv2_first_charger_spot;
            ...
            %%
            % time = d/userConfig.uav_return_speed;% 60 km/ hr
            time = d/userConfig.uav_return_speed + wait_asv_move_time; % 60 km/ hr wait for asv move time
            % time_charging = time;   
            time_charging = d/userConfig.uav_return_speed;  % do not include the wait ASV moving time
...
%                     end   
                    asv1_travel_time = userConfig.asv_travel(1);
                    asv2_travel_time = userConfig.asv_travel(2);
                    asv1_start_time = asv1_travel_time.data(:,1); % start moving time
                    asv2_start_time = asv2_travel_time.data(:,1);
                    asv1_charger_time = asv1_start_time(2:end);% remove first element  
                    asv2_charger_time = asv2_start_time(2:end);
                    
                    charger_idx_1 = findInsertPosition(asv1_charger_time,time);  % check the order of the variable b in A array "compare_position"
                    charger_idx_2 = findInsertPosition(asv2_charger_time,time);
                    %%
...
%                     asv2_xy = userConfig.asv2_xy(1+charger_idx,:);
                    ...
                    asv1_xy = userConfig.asv1_xy(1+charger_idx_1,:);
                    asv2_xy = userConfig.asv2_xy(1+charger_idx_2,:);
                    ...
                    if arrive_asv1_time < arrive_asv2_time
                        ...
                        leave_time = arrive_asv1_time+userConfig.charging_time_drone;
                        C = [arrive_asv1_time leave_time];
                        [asv1_1, asv1_2, space_remain] = insertIntervals(asv1_1, asv1_2, C);
                        if space_remain == 0
                            % mean no space in the asv 1 so go to the asv 2
                            leave_time = arrive_asv2_time+userConfig.charging_time_drone;
                            C = [arrive_asv2_time leave_time];
                            ...
                            record_uav_usv(end + 1, :) = [s, dist_asv2, arrive_asv2_time, uavpos, asv2_row_indices, nth_charging_spot, leave_time];
                            which_asv = 2;
                        else
                            if space_remain == 1 
                            ...
                            record_uav_usv(end + 1, :) = [s, dist_asv1, arrive_asv1_time, uavpos, asv1_row_indices, nth_charging_spot, leave_time];
                            which_asv = 1;
                        end                       
                    else
                    ...
                    if which_asv == 1
                        return_flight_time = return_to_asv1_flight_time;                       
                    elseif which_asv == 2
                        return_flight_time = return_to_asv2_flight_time;  
                    end
                    
                    %% check whether uav can fly back to charger
                    check_battery_level = time_charging + return_flight_time;              
                    if check_battery_level > userConfig.uav_battery_limit % 0.7 hour
                        uav_cannot_fly_back_penalty = uav_cannot_fly_back_penalty + 1;
                        %disp('cannot fly back, please add penalty or give up this gene') 
                    end
                    
                    
                    %% check take off when asv is moving 
                    oringal_take_off_time = time + charging_time + return_flight_time;
                    
                    asv_travel_time = userConfig.asv_travel(which_asv);  % only consider two charging cycle (second and third)
                    asv_travel_time_after_first = asv_travel_time.data(2:end,:);  % only consider after first travel, 
                    [fly_during_ASV_moving take_off_from_ASV_time, charger_spot]= checkRange(asv_travel_time_after_first, oringal_take_off_time); % since input matrix start from 2 
                    % so here nth_charging_spot = 1,  is the 2nd charging spot, use asv_positions(3:end,:) to match
    
                    if fly_during_ASV_moving == 1  % please wait, until boat stop and then take off.
                        % take_off_from_ASV_time = ASV_stop_moving_time; % will be the real take off time
                        if which_asv == 1
                            asv_positions = userConfig.asv1_xy; % 4x2 start, 1st 2nd 3rd x  XY position 
                        elseif which_asv == 2
                            asv_positions = userConfig.asv2_xy;
                        end
                        asv_positions = asv_positions(3:end,:); % remove start and 1st point,   only keep 2nd and 3rd charging position 
                        asv_xy_position = asv_positions(charger_spot,:); % new start position, use this position to calculate 2nd_return_flight_time
                        % return_flight_time is that the uav fly to the asv
                        % second_return_flight_time is that the uav fly from NEW asv position to work
                        % double_return_flight_time is that uav fly to the asv and back to work
                        
                        % Check for rows in A that match vector B
                        new_asv_indices = ismember(xy, asv_xy_position, 'rows'); %ismember(A, B, 'rows');                      
                        % Find the row indices where the match occurs 
                        new_asv_row_indices = find(new_asv_indices);
                        % Calculate distance between UAVs and USVs
                        new_dist_asv = dmat(uavpos,new_asv_row_indices);
                        % Calculate flight time 
                        second_return_flight_time = new_dist_asv/userConfig.uav_return_speed; % 60 km/ hr   
                        % second_return_flight_time is that the uav fly from NEW asv position to work
                        
                        record_uav_usv(end, 2) = 0.5*record_uav_usv(end, 2); % distance half, because go and back are separate
                        record_uav_usv(end + 1, :) = [s, 0.5*new_dist_asv, oringal_take_off_time, uavpos, new_asv_row_indices, nth_charging_spot, take_off_from_ASV_time];
                                 
                        % time, time_charging (battery level)
                        double_return_flight_time = return_flight_time + second_return_flight_time;  % go and back are different
                        return_flight_time = second_return_flight_time;  % back to work is different, so time_charging (battery level) need to consider
                    else
                        double_return_flight_time = return_flight_time*2; % time 2 because go and back
                    end
...
%%                    
                    %time = time + charging_time;
                    time = time + charging_time + double_return_flight_time; % lifan add 
                    time_end_charging(battery_life_count) = time_start_charging(battery_life_count) + charging_time; % time finished charging
                    % time_charging = dmat(pRoute(k),pRoute(k+1))/delta_v;   % reset the  battery level
                    time_charging = dmat(pRoute(k),pRoute(k+1))/delta_v + return_flight_time;  % lifan add consider one return battery level
                ...
                d = d + dmat(pRoute(k),pRoute(k+1)); % distance for mission
                time = time + dmat(pRoute(k),pRoute(k+1))/delta_v; % time for travel
                count = count + 1;
            end
%             d = d + dmat(pRoute(rng(s,2)),pRoute(rng(s,1))); % make it a circle
            count = count + 1;
...
%%        
        ...
        uav_cannot_fly_back_penalty_record(p) = uav_cannot_fly_back_penalty;
...
%         end
        ...
        mission_time_record = [mission_time_record; mission_time];
        ...
        % fitness function
        %fun(p) = alpha_ratio(iter,1)*energy_cost(p) + alpha_ratio(iter,5)*speed_limit_penalty(p); + uav_cannot_fly_back_penalty_record(p)
        fun(p) = alpha_ratio(iter,1)*energy_cost(p) + 0.01*total_ds(p) + uav_overlap_penalty_record(p)+ 1.5*uav_cannot_fly_back_penalty_record(p);
...
%     if minFun < globalMin
        ...
        min_uav_cannot_fly_back_penalty =  uav_cannot_fly_back_penalty_record(index); % lifan add
        min_mission_time_record = mission_time_record(index, :);
        fprintf('uav_overlap_penalty_record ( %d ) = %d uav_cannot_fly_back_penalty_record = %d  \n', index, uav_overlap_penalty_record(index), uav_cannot_fly_back_penalty_record(index)); % lifan add , should be zero
...
if nargout
    resultStruct = struct( ...
        ...
        'minTime',     minTime, ...  % lifan remove , original is 'find_mission_time'
        'minEnergy',   minEnergy, ...
        'minDist',     minDist, ...
        'globalMin',    globalMin, ...
        'minds',        minds, ...
        ...
        'min_mission_time_record', min_mission_time_record, ... % lifan added
        'min_uav_cannot_fly_back_penalty', min_uav_cannot_fly_back_penalty, ... % lifan added
...
function position = findInsertPosition(A, b)
    % Check if b is smaller than the first element
    if b < A(1)
        position = 1;
        return;
    end
    % Check if b is greater than the last element
    if b >= A(end)
        position = length(A)+1;
        return;
    end    
