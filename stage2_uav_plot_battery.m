numTarChargers = userConfig.numTarChargers;
...
%     uav_path_y = [start_pCharger(ic,2); a.xy(a.charging_location(charger_location_ind(ic,1):charger_location_ind(ic,2)),2)];
    ...
    %plot(uav_path_x, uav_path_y,'linestyle','none','marker','x','LineWidth',4,'MarkerSize',15,'Color',clr(s+ic,:)) % include start point
    plot(uav_path_x(2:end), uav_path_y(2:end),'linestyle','none','marker','x','LineWidth',4,'MarkerSize',15,'Color',clr(s+ic,:)) % remove start point
...
plot(start_point(1,1),start_point(1,2),'pentagram','MarkerSize',15,'LineWidth',1,'Color',dark_red); hold on;   % UAV1 
plot(start_point(2,1),start_point(2,2),'pentagram','MarkerSize',15,'LineWidth',1,'Color',dark_red,'HandleVisibility','off')  % UAV2 
plot(start_point(3,1),start_point(3,2),'pentagram','MarkerSize',15,'LineWidth',1,'Color',dark_red,'HandleVisibility','off')  % UAV3
...
title(hAx,sprintf('UAV Working Distance = %1.2f, Return Distance = %1.2f',a.minDist,a.minds));
...
% h1_flag == 0 && nth_charger_color == 1

for i = 1:uav_return_flight_number   
    uav_nth = a.charging_arrangement_uav.data(i,1);
    arrived_time = a.charging_arrangement_uav.data(i,3);
    %ended_time = arrived_time + 2; % add charging time
    ended_time = a.charging_arrangement_uav.data(i,7); % add charging time
    nth_charging_spot = a.charging_arrangement_uav.data(i,6);
    %uav_indice = a.charging_arrangement_uav.data(i,4);
    %asv_indice = a.charging_arrangement_uav.data(i,5);
    %uav_pos = a.xy(uav_indice,:);
    %asv_pos = a.xy(asv_indice,:);
    
    xy_1 = [arrived_time nth_charging_spot];
    xy_2 = [ended_time nth_charging_spot];
    
    line = [xy_1;xy_2];
    
    %plot(uav_pos(1),uav_pos(2),'kx', 'linewidth', 2,'MarkerSize',10,'MarkerFaceColor','k')
    
    % plot(line(:,1),line(:,2),'linewidth',10,'Color',clr(uav_nth,:));     
    if h1_flag == 0 && uav_nth == 1
        plot(line(:,1),line(:,2),'linewidth',10,'Color',clr(uav_nth,:)); 
        h1_flag = 1;
    elseif h2_flag == 0 && uav_nth == 2
        plot(line(:,1),line(:,2),'linewidth',10,'Color',clr(uav_nth,:)); 
        h2_flag = 1;
    elseif h3_flag == 0 && uav_nth == 3
        plot(line(:,1),line(:,2),'linewidth',10,'Color',clr(uav_nth,:));
        h3_flag = 1;
    else
        plot(line(:,1),line(:,2),'linewidth',10,'Color',clr(uav_nth,:),'HandleVisibility','off'); 
    end
    hold on;   
end


%% plot asv travel time 

green = [0 1 0];
orang = [0.9290 0.6940 0.1250];

asv_color = [green;orang];

h1_flag = 0;
h2_flag = 0;

for i = 1:numCharger  % green and orange
    
    asv_travel_time = asv_travel(i).data;
    
    for j = 1:length(asv_travel_time)  % 1~3
        start_t = asv_travel_time(j,1);
        end_t = asv_travel_time(j,2);
        xy_1 = [start_t 1+2*(i-1)];
        xy_2 = [end_t 1+2*(i-1)];
        line = [xy_1;xy_2];
        
        if h1_flag == 0 && i == 1
            plot(line(:,1),line(:,2),'linewidth',5,'Color',asv_color(i,:));
            h1_flag = 1;
        elseif h2_flag == 0 && i == 2
            plot(line(:,1),line(:,2),'linewidth',5,'Color',asv_color(i,:));
            h2_flag = 2;
        else
            plot(line(:,1),line(:,2),'linewidth',5,'Color',asv_color(i,:),'HandleVisibility','off');
        end
        
        hold on;    
        xy_1 = [start_t 2+2*(i-1)];
        xy_2 = [end_t 2+2*(i-1)];
        line = [xy_1;xy_2];
        plot(line(:,1),line(:,2),'linewidth',5,'Color',asv_color(i,:),'HandleVisibility','off'); 
        hold on;        
...
xlabel('Time (h)')
ylabel('nth charging spot on ASV')
grid on;
legend('UAV#1','UAV#2','UAV#3','charger#1','charger#2')
% Set y-axis ticks to show only integers
% yticks(round(min(a.charging_arrangement_uav.data(:, 6))):1:round(max(a.charging_arrangement_uav.data(:, 6))));
yticks(1:4);
yticklabels({'ASV1-1','ASV1-2','ASV2-1','ASV2-2'});

ylim([0.9, 4.1]);

title("UAV charging period and ASV moving period")