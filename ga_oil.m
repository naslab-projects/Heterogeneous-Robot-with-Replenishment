function varargout = ga_oil(varargin)
    ...
    for p = 1:popSize
        ...
        for s = 1:nSalesmen
            ...
            for k = rng(s,1):rng(s,2)-1  % loop for each battery life; record first
                ...
                if time_charging > pBatteryLife(s,1) % if the battery is enough to make this travel
...
%             d = d + dmat(pRoute(rng(s,2)),pRoute(rng(s,1))); % make it a circle
        ...
        for ic = 1:numTarChargers
            ...
            time_charger_travel(charger_count) = start_mat(new_order_pStation(charger_location_ind(ic,1)),2*1+1)/delta_vc;
