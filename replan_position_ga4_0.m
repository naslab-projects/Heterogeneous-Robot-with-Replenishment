function varargout = replan_position_ga4_0(varargin)
...
numIter     = configStruct.numIter/2;
...
for iter = 1:numIter
    ...
    for p = 1:popSize
        ...
        for s = 1:nSalesmen
            d = start_mat(pRoute(rng(s,1)),(s-1)*2+1); % add starting point
            time = d/start_mat(pRoute(rng(s,1)),(s-1)*2+2); % add time start point
            time_charging = time;          
            for k = rng(s,1):rng(s,2)-1  % loop for each battery life; record first
                ...
                if time_charging > pBatteryLife(s,man_battery_count(s)+1) % if the battery is enough to make this travel
                    ...
                    time_charging = dmat(pRoute(k),pRoute(k+1))/delta_v;
...
%             d = d + dmat(pRoute(rng(s,2)),pRoute(rng(s,1))); % make it a circle
            ...
            mission_time(s) = time;
            temp_battery_life_distance(s) = battery_life_count;
        end
        battery_life_distance{p} = temp_battery_life_distance;
        % remove zeros
        ...
        for ic = 1:numTarChargers
            ...
            time_charger_travel(charger_count) = start_mat(new_order_pStation(charger_location_ind(ic,1)),2*nSalesmen+ic)/delta_vc;
            ...
            for jc = charger_location_ind(ic,1):charger_location_ind(ic,2)-1
                time_charger_travel(charger_count) = dmat(new_order_pStation(jc),new_order_pStation(jc+1))/delta_vc;
...
%         mission_time = mission_time + pp_time;
        ...
        % fitness function
        fun(p) = alpha_ratio(iter,1)*energy_cost(p)*10 + alpha_ratio(iter,4)*mean(time_charger_travel); % distance and priority sub-objective and charger sub-objective
