...
% load('auv_time.mat');
% auv1_total_travel_time = AUV1(1) + AUV1(3) + AUV1(5);
% auv2_total_travel_time = AUV2(1) + AUV2(4) + AUV2(6);
% auv3_total_travel_time = AUV3(1) + AUV3(3) + AUV3(5);
...
v = 30/scala; %3/scala; % worker speed   3km/hr with 2hr safety level   % change the drone speed 
...
btl = 0.4; % 2; %8, 10; % battery life  max 12 hr battery 
...
if 1 % "1" for single run, "0" for multiple runs using parallel computing
    a = ga4_3_drone_CPP(userConfig); % use instant charge and speed limit constraint
    Run_time = toc/60
