function varargout = ga6_0_drone_CPP(varargin)
...
%                     asv2_xy = userConfig.asv2_xy(1+charger_idx,:);
                    ...
                    [fly_during_ASV_moving take_off_from_ASV_time, nth_charging_spot]= checkRange(asv_travel_time_after_first, oringal_take_off_time); % since input matrix start from 2 
                    ...
                    if fly_during_ASV_moving == 1  % please wait, until boat stop and then take off.
                        ...
                        asv_xy_position = asv_positions(nth_charging_spot,:); % new start position, use this position to calculate 2nd_return_flight_time
...
%         end
        ...
        fun(p) = alpha_ratio(iter,1)*energy_cost(p) + 0.01*total_ds(p) + uav_overlap_penalty_record(p)+ uav_cannot_fly_back_penalty_record(p);
