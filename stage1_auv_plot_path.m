function [traj_segment,time_given_charger, traj_worker_nth] = stage1_auv_plot_path(a,numTarChargers,userConfig,start_point,start_pCharger,G,start_node_th,points_to_remove)
%% Plot
N = length(a.optRoute);
...
%     battery_history(s,ceil(time)) = 20 - time;
    for k = rng(s,1):rng(s,2)-1
        ...
        if time_charging > batteryLife(s,man_battery_count(s)+1) % if the battery is enough to make this travel
            ...
            traj_worker_nth(battery_life_count) = s; % lifan for plot battery
...
%     battery_history = battery_history(:,1:ceil(totalTime));
    ...
    fileName = ['auv', num2str(s), '.mat'];  % autonmous uderwater vehicle 
    auv_x = [start_point(s,1); a.xy(rte,1)];
    auv_y = [start_point(s,2); a.xy(rte,2)];
    save(fileName, 'auv_x', 'auv_y');    
end
time_temp;
